...
%--------------------------------------------------------------------------
%
% modelForcing_oscillatoryLinearDecay.m
% 
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function f = modelForcing_oscillatoryLinearDecay(t,param)
...
if length(t) > 1,
    ind = find( t <= tau );
    f(ind) = F0*( 1-t(ind)/tau ) .* ( alpha*sin(omega*t(ind))+1 );
    ind = find( t > tau );
    f(ind) = 0;
else
    if t <= tf,
        f = F0*( 1-t/tau ) * ( alpha*sin(omega*t)+1 );
    else
        f = 0;
    end;
end;
