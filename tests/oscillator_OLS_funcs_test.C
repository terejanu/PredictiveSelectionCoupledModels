...
//********************************************************************
bool test_OLS_func();
bool test_OLS_int_exact();
bool test_OLS_int_energy();
...
int main(int argc, char* argv[])
{
  ...
  // tests OLS_func
  TST_MACRO( test_OLS_func(),
	     "oscillatorLinearSpring_func",
	     "" );

  // tests OLS_int_exact
  TST_MACRO( test_OLS_int_exact(),
	     "oscillatorLinearSpring_int",
	     "exact solution" );

  // tests OLS_int_energy
  TST_MACRO( test_OLS_int_energy(),
	     "oscillatorLinearSpring_int",
	     "const energy" );
...
bool test_OLS_func()
{
  ...
  double exact_f[2] = { ic_y[1], -param_c/__OSCILLATOR_MASS*(ic_y[0]+ic_y[1]) };
...
bool test_OLS_int_energy()
{
  ...
  map_scn[ __OLS_SCN_TIME ] = scn_time_1;
  ...
  double energy1 = pow( final_y1, 2.0 ) / 2.0 +
    pow( final_y0, 2.0 ) / 2.0 * param_k10;

  // get the energy at time: sc_time_2
  double scn_time_2 = 1.1;
  map_scn[ __OLS_SCN_TIME ] = scn_time_2;
